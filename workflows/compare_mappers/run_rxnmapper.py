"""Run RXNMapper v1 on unmapped reactions in batches.

Reads unmapped_reactions.txt, runs RXNMapper v1 in batches, and writes
mapped reaction SMILES to rxnmapper_mapped.txt (one per line, empty
string on failure).  Optionally saves a timing JSON file for use by
``compile_speed_table.py``.

Requires: rxnmapper (pip install rxnmapper[rdkit])
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run RXNMapper v1 on unmapped reactions"
    )
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        "--output",
        default=str(Path(__file__).resolve().parent / "rxnmapper_mapped.txt"),
