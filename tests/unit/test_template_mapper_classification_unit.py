...
def _make_pattern(
    ...
    subclass_id: str = "0",
    subsubclass_id: str = "",
    rxno_classification: list[str] | None = None,
...
class TestMakeCompositeSmirksPattern:
    ...
    def test_rxno_classification_flattened_deduplicated(self):
        p1 = _make_pattern(rxno_classification=["RXNO:0000001", "RXNO:0000002"])
        p2 = _make_pattern(rxno_classification=["RXNO:0000002", "RXNO:0000003"])
        composite = _make_composite_smirks_pattern(
            [p1, p2], "[C:1]>>[C:1]", p1["rdc_rxn"]
        )
    ...
    def test_rxno_classification_preserves_order(self):
        p1 = _make_pattern(rxno_classification=["RXNO:0000003", "RXNO:0000001"])
        p2 = _make_pattern(rxno_classification=["RXNO:0000002"])
        composite = _make_composite_smirks_pattern(
            [p1, p2], "[C:1]>>[C:1]", p1["rdc_rxn"]
        )
        assert composite["rxno_classification"] == [
...
class TestInitializeSmirksPatterns:
    ...
    def test_rxno_classification_is_list_of_strings(self, mapper):
        """rxno_classification should be a list of ID strings."""
        mapper._initialize_smirks_patterns()
        patterns = mapper._initialized_smirks_patterns
        assert patterns is not None
        for p in patterns:
            ...
            for rxno_id in p["rxno_classification"]:
                assert isinstance(rxno_id, str)
                assert rxno_id.startswith("RXNO:")
...
class TestMapReactionClassificationInfo:
    ...
    def test_rxno_classification_is_list_of_strings_in_result(self, mapper):
        """rxno_classification in classification_info should be list of strings."""
        result = mapper.map_reaction(
            "[CH2:0]([Cl])[c:1]1ccccc1.[CH2:2]([O:3][H])>>[CH2:0]([O:3])[c:1]1ccccc1.[Cl][H]"
        )
        if result.selected_mapping:
            for entries in result.classification_info.values():
                for entry in entries:
                    ...
                    for rxno_id in entry["rxno_classification"]:
                        assert isinstance(rxno_id, str)
                ...
                "[C:1]>>[C:1]": [
                    {
                        ...
                        "subsubclass_id": "",
                        "superclass_id": "1",
                        "rxno_classification": ["RXNO:0000001"],
                    }
                ]
            },
        )
